...
import ceni.system.votesync.model.entity.election.result.provisional.details.ProvisionalProvincialResultDetails;
import ceni.system.votesync.service.FileStorageService;
import ceni.system.votesync.service.NumberStringFormatter;

@Service
public class ProvincialResultPdf extends ResultPdf {

    private FileStorageService fileStorageService;

    public ProvincialResultPdf(FileStorageService fileStorageService) {
        this.fileStorageService = fileStorageService;
    }

    public ByteArrayOutputStream generate(List<ProvisionalProvincialResult> results) throws IOException {
        try (PDDocument document = new PDDocument(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {

            for (ProvisionalProvincialResult result : results) {
                // Create an A4 page
                String documentTitle = "VOKATRA VONJIMAIKA AMIN'NY FIFIDIANANA: " + result.getElection();
    ...
    private void addResultDetailsSection(PDPageContentStream contentStream, PDPage page,
            ProvisionalProvincialResult result) throws IOException {
        float fontSize = 9;
        ...
        contentStream.showText("Faritany: " + result.getName());
    ...
    private void addTableDetails(PDDocument document, PDPage page, PDPageContentStream contentStream,
            float startY, ProvisionalProvincialResult result) throws IOException {
        ...
        for (int row = 0; row < result.getDetails().size(); row++) {
            ...
            ProvisionalProvincialResultDetails details = result.getDetails().get(row);

            // Column 1: Numéro (centered)
            drawCenteredText(contentStream, startX, nextY, columnWidths[0], cellHeight,
                    details.getCandidateNumber().toString(), PDType1Font.HELVETICA_BOLD, Color.BLACK, fontSize);
            ...
            drawRightAlignedText(contentStream, startX, nextY, columnWidths[3], cellHeight,
                    String.format("%.2f", rate) + "%", PDType1Font.HELVETICA, Color.BLACK, fontSize);
