...
@Service
public class GlobalResultPdf extends ResultPdf {

    private FileStorageService fileStorageService;

    public GlobalResultPdf(FileStorageService fileStorageService) {
        this.fileStorageService = fileStorageService;
    }

    public ByteArrayOutputStream generate(List<ProvisionalGlobalResult> results) throws IOException {
        try (PDDocument document = new PDDocument(); ByteArrayOutputStream out = new ByteArrayOutputStream()) {

            for (ProvisionalGlobalResult result : results) {
                // Create an A4 page
    ...
    private void addResultDetailsSection(PDPageContentStream contentStream, PDPage page,
            ProvisionalGlobalResult result) throws IOException {
        float fontSize = 9;
    ...
    private void addTableDetails(PDDocument document, PDPage page, PDPageContentStream contentStream,
            float startY, ProvisionalGlobalResult result) throws IOException {
        ...
        for (int row = 0; row < result.getDetails().size(); row++) {
            ...
            ProvisionalGlobalResultDetails details = result.getDetails().get(row);
