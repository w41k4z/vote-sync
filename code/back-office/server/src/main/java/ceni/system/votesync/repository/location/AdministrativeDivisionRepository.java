...
public interface AdministrativeDivisionRepository<T extends AdministrativeDivision>
    ...
    @Query("SELECT new ceni.system.votesync.model.location.AdministrativeDivision(a.id, a.name, a.code, a.upperDivisionId) FROM #{#entityName} a")
    List<T> findAllWithoutGeoJson();

    @Query("SELECT new ceni.system.votesync.model.location.AdministrativeDivision(a.id, a.name, a.code, a.upperDivisionId) FROM #{#entityName} a WHERE a.upperDivisionId = ?1")
